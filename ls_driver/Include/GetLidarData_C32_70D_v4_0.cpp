...
GetLidarData_C32_70D_v4_0::GetLidarData_C32_70D_v4_0()
{
	float theta70D[32] = {
	...
	for (int i = 0; i < 32; i++)
	{
		cosTheta[i] = cos(theta70D[i] * PI / 180);
		sinTheta[i] = sin(theta70D[i] * PI / 180);
	}
...
void GetLidarData_C32_70D_v4_0::LidarRun()
{
	while (true)
	{
		if (isQuit)
...
void GetLidarData_C32_70D_v4_0::handleSingleEcho(unsigned char* data)
{
		...
		for (int i = 0; i < 12; i++)
		{
			...
			if (BlockAngle.size() >= 2)
			{
				for (int j = 0; j < 32; j++)
				{
					...
					if (m_DataT[j].ID == 29 || m_DataT[j].ID == 6 || m_DataT[j].ID == 14 || m_DataT[j].ID == 22 ||
						m_DataT[j].ID == 30 || m_DataT[j].ID == 7 || m_DataT[j].ID == 15 || m_DataT[j].ID == 23)
					{
						m_angle += offsetAngle;
					}
...
void GetLidarData_C32_70D_v4_0::handleDoubleEcho(unsigned char* data)
{
	if (data[0] == 0xff && data[1] == 0xee)
	{
...
m_PointXYZ GetLidarData_C32_70D_v4_0::XYZ_calculate(int ID, double H_angle, double Distance)
{
	m_PointXYZ point;
