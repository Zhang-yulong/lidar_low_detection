...
void GetLidarData_C16_v3_0::handleSingleEcho(unsigned char* data)
{
	if (data[0] == 0xff && data[1] == 0xee)
	{
		...
		for (int i = 0; i < 12; i++)
		{
			...
			{
				m_DataT[j].ID = j % 16;
				m_DataT[j].Distance = (data[5 + 3 * j + 100 * i] * 256 + data[4 + 3 * j + 100 * i]) * IDistanceACC_C16_v3_0;
	...
	if (data[0] == 0xff && data[1] == 0xee)
	{
		...
		for (int i = 0; i < 12; i += 2)
		{
			...
			for (int j = 0; j < 32; j++)
			{
				...
				m_DataT_d[j].ID = j % 16;
				m_DataT_d[j].Distance = (data[5 + 3 * j + 100 * (i + 1)] * 256 + data[4 + 3 * j + 100 * (i + 1)]) * IDistanceACC_C16_v3_0;
