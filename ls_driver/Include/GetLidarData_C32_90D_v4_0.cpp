...
GetLidarData_C32_90D_v4_0::GetLidarData_C32_90D_v4_0()
{
	//C32 90°的封包顺序
	float theta90[32] = {
	...
	for (int i = 0; i < 32; i++)
	{
		cosTheta[i] = cos(theta90[i] * PI / 180);
		sinTheta[i] = sin(theta90[i] * PI / 180);
	}
...
void GetLidarData_C32_90D_v4_0::LidarRun()
{
	while (true)
	{
		if (isQuit)
...
void GetLidarData_C32_90D_v4_0::handleSingleEcho(unsigned char* data)
{
...
void GetLidarData_C32_90D_v4_0::handleDoubleEcho(unsigned char* data)
{
	if (data[0] == 0xff && data[1] == 0xee)
	{
...
m_PointXYZ GetLidarData_C32_90D_v4_0::XYZ_calculate(int ID, double H_angle, double Distance)
{
	m_PointXYZ point;
