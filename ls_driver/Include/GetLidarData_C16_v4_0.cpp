...
GetLidarData_C16_v4_0::GetLidarData_C16_v4_0()
{
	float theta[16] = { -16, 0, -14, 2, -12, 4, -10, 6, -8, 8, -6, 10, -4, 12, -2, 14 };
	for (int i = 0; i < 16; i++)
	{
...
void GetLidarData_C16_v4_0::LidarRun()
{
	while (true)
	{
		if (isQuit)
...
void GetLidarData_C16_v4_0::handleSingleEcho(unsigned char* data)
{
		...
		for (int i = 0; i < 12; i++)
		{
			...
			if (BlockAngle.size() >= 2)
			{
				for (int j = 0; j < 32; j++)
				{
					...
					if(!isSetOriAngle)
					{
						endFrameAngle = m_DataT[j].H_angle;
						isSetOriAngle = true;
					}
					...
					{
						if ((m_DataT[j].H_angle - endFrameAngle > 0) && abs(m_DataT[j].H_angle - endFrameAngle) < 10)
						{
...
void GetLidarData_C16_v4_0::handleDoubleEcho(unsigned char* data)
{
	if (data[0] == 0xff && data[1] == 0xee)
	{
...
m_PointXYZ GetLidarData_C16_v4_0::XYZ_calculate(int ID, double H_angle, double Distance)
{
	m_PointXYZ point;
