...
GetLidarData_CH32::GetLidarData_CH32()
{
	for (int i = 0; i < 32; i++)
	{
		cosTheta[i] = cos((-6.67 + i*0.33)* PI / 180);
		sinTheta[i] = sin((-6.67 + i*0.33) * PI / 180);
	}
	for (int i = 0; i < 32; i = i + 4)
	{
		Theat_T[i] = sin((-6.67 + i*0.33)*PI / 180);
		Theat_T[i + 1] = sin((-6.67 + i*0.33)*PI / 180);
		Theat_T[i + 2] = sin((-6.67 + i*0.33)*PI / 180);
		Theat_T[i + 3] = sin((-6.67 + i*0.33)*PI / 180);
...
void GetLidarData_CH32::LidarRun()
{
	while (true)
	{
		if (isQuit)
...
void GetLidarData_CH32::handleSingleEcho(unsigned char* data)
{
...
void GetLidarData_CH32::handleDoubleEcho(unsigned char* data)
{
	m_UTC_Time.second = data[1199];
	struct tm t;
	t.tm_sec = m_UTC_Time.second;
...
m_PointXYZ GetLidarData_CH32::XYZ_calculate(int ID, double H_angle, double Distance)
{
	m_PointXYZ point;
