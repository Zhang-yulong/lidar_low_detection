...
GetLidarData_CH128S1::GetLidarData_CH128S1() {

    for (int i = 0; i < 128; i++) {
		sinTheta_1[i] = sin(G_Angle_CH128S1[i / 4] * PI / 180);
        sinTheta_2[i] = sin((i % 4) * (-0.17) * PI / 180);

		cosTheta_1[i] = cos(G_Angle_CH128S1[i / 4] * PI / 180);
...
void GetLidarData_CH128S1::LidarRun() {
    while (true) {
        if (isQuit) {
...
void GetLidarData_CH128S1::getOffsetAngle(std::vector<int> OffsetAngleValue)
{
	double prismAngle[4];
	...
	if (abs(OffsetAngleValue[0]) != 0)
	{
		...
		for (int i = 0; i < 128; i++)
		{
			...
			if (i / 4 % 2 == 0)
			{
				sinTheta_1[i] = sin((G_Angle_CH128S1[i / 4] + PrismOffsetVAngle) * PI / 180);
				cosTheta_1[i] = cos((G_Angle_CH128S1[i / 4] + PrismOffsetVAngle) * PI / 180);
			}
			else
			{
				sinTheta_1[i] = sin(G_Angle_CH128S1[i / 4] * PI / 180);
...
void GetLidarData_CH128S1::handleSingleEcho(unsigned char *data) {
...
void GetLidarData_CH128S1::handleDoubleEcho(unsigned char *data) {
...
m_PointXYZ GetLidarData_CH128S1::XYZ_calculate(int ID, double H_angle, double Distance) {
    m_PointXYZ point;
